...
# --- 8. LOT ΠΑΡΑΓΩΓΗΣ (ΜΕ DROP-DOWN ΠΕΛΑΤΟΛΟΓΙΟ) ---
    ...
    # --- 4. ΙΣΤΟΡΙΚΟ & ΔΙΑΧΕΙΡΙΣΗ (ΠΛΗΡΗΣ ΔΙΟΡΘΩΜΕΝΟΣ ΚΩΔΙΚΑΣ) ---
st.divider()
...
if res_log.data:
    ...
    # --- Α. ΜΑΖΙΚΗ ΔΙΑΓΡΑΦΗ (INTERACTIVE SELECTION) ---
    st.markdown("### 🗑️ Μαζική Διαχείριση")
    st.caption("Τσεκάρετε τις γραμμές που θέλετε να διαγράψετε οριστικά.")

    # Δημιουργία Interactive Πίνακα για επιλογή
    event = st.dataframe(
        ...
        column_config={
            "id": None, "prod_date": "Ημερομηνία", "prod_time": "Ώρα",
            "customer": "Πελάτης", "cocktail_name": "Cocktail", "pieces": "Τμχ", "lot_cocktail": "LOT"
        }
    )
    ...
    if selected_rows:
        ...
        if st.button(f"🗑️ Οριστική Διαγραφή ({len(selected_rows)}) επιλεγμένων", type="primary"):
    ...
    # --- Β. Η ΔΙΚΗ ΣΟΥ ΛΟΓΙΚΗ (FILTER & EDIT BATCH) ---
    ...
    if sel_hist_date:
        ...
        if selected_batch != "-- Επιλέξτε Παραγωγή --":
            ...
            # --- ΕΝΟΤΗΤΑ ΕΚΤΥΠΩΣΗΣ (ΔΙΟΡΘΩΜΕΝΗ ΕΥΘΥΓΡΑΜΜΙΣΗ) ---
            if st.button("🖨️ Εκτύπωση PDF"):
                # Εδώ η ευθυγράμμιση είναι κρίσιμη
                html_pro = f"""
                <html>
                <body>
                    <h2>Δελτίο Παραγωγής: {selected_batch}</h2>
                    <p>Ημερομηνία: {base_data['Ημερομηνία']} | Τεμάχια: {new_pcs}</p>
                    <table border='1' style='width:100%; border-collapse: collapse;'>
                        <tr><th>Υλικό</th><th>ML</th><th>Lot</th><th>Λήξη</th></tr>
                        {''.join([f"<tr><td>{x['ing']}</td><td>{x['ml']:.0f}</td><td>{x['lot']}</td><td>{x['exp']}</td></tr>" for x in final_updated])}
                    </table>
                </body>
                </html>
                """
                st.download_button("📥 Λήψη PDF", data=html_pro, file_name=f"Lot_{new_lot_c}.html", mime="text/html")
else:

...
# ==========================================
# 1. ΣΥΝΔΕΣΗ ΜΕ GOOGLE SHEETS API (SECURE)
# ==========================================
    ...
    try:
        ...
        def fetch_tab(name, expected_cols):
            try:
                ...
                # Καθαρισμός στηλών από κενά (π.χ. "ID " -> "ID")
                df.columns = [str(c).strip() for c in df.columns]
                
                # Διασφάλιση ότι υπάρχουν όλες οι στήλες
        ...
        # ΠΡΟΣΟΧΗ: Αυτά πρέπει να υπάρχουν ως επικεφαλίδες στο Google Sheet
...
def check_password():
    """Σύστημα Password με session state"""
...
# CSS για βελτίωση της εμφάνισης
st.markdown("""
    <style>
...
with st.sidebar:
    ...
    if st.button("🔄 Ανανέωση από Google Sheets"):
        st.cache_data.clear()
        st.rerun()

    current_user = st.selectbox("👤 Είσαι ο:", ["Χρήστης Α", "Χρήστης Β"])
    st.info(f"Συνδεδεμένος: {current_user}")
    ...
    page = st.radio("Μενού:", [
        ...
        "🌐 Shop Sync", "📦 Lot Παραγωγής", "📈 Dashboard", "🧼 Συντήρηση & HACCP"
    ])
    
    # Φόρτωση Δεδομένων από Sheets
    df_ing, df_rec, df_orders, df_history = load_data_from_sheets()
...
# Βασικές λίστες επιλογών για την υπόλοιπη εφαρμογή
...
# ==============================================================================
    ...
    if past_files:
        ...
        if sel_hist_date:
            ...
            # --- ΝΕΟ ΤΜΗΜΑ: ΗΜΕΡΗΣΙΟ ΦΥΛΛΟ ΠΑΡΑΓΩΓΗΣ (ΧΩΡΙΣ ΥΛΙΚΑ - ΧΩΡΙΣΜΕΝΟ ΑΝΑ COCKTAIL) ---
            # 1. Απομόνωση μοναδικών παραγωγών και ομαδοποίηση
            ...
            # 3. Δημιουργία πινάκων ανά Cocktail
            for cocktail in df_grouped['Cocktail'].unique():
                # Φιλτράρισμα δεδομένων μόνο για το συγκεκριμένο cocktail
                ...
                # Προσθήκη μερικού συνόλου για το cocktail
                html_daily += f"""
            ...
            # 4. Προσθήκη Γενικού Συνόλου Ημέρας στο τέλος

...
# --- 8. LOT ΠΑΡΑΓΩΓΗΣ (ΜΕ DROP-DOWN ΠΕΛΑΤΟΛΟΓΙΟ) ---
    ...
    # --- 4. ΙΣΤΟΡΙΚΟ & ΔΙΑΧΕΙΡΙΣΗ (ΜΕ INTERACTIVE DELETE & EDIT BATCH) ---
    ...
    if res_log.data:
        ...
        # --- Α. ΝΕΑ ΛΕΙΤΟΥΡΓΙΑ: ΜΑΖΙΚΗ ΔΙΑΓΡΑΦΗ ΜΕ CHECKBOXES ---
        st.subheader("🗑️ Μαζική Διαγραφή")
        st.caption("Επιλέξτε τις γραμμές που θέλετε να διαγράψετε οριστικά από τη βάση.")
        
        # Εμφάνιση Interactive Πίνακα για μαζική διαγραφή
        event = st.dataframe(
            ...
            column_config={
                "id": None, 
                "prod_date": "Ημερομηνία", "prod_time": "Ώρα", "customer": "Πελάτης",
                "cocktail_name": "Cocktail", "pieces": "Τμχ", "lot_cocktail": "LOT"
            }
        )
        ...
        if selected_rows:
            ...
            if st.button(f"🗑️ Οριστική Διαγραφή ({len(selected_rows)}) επιλεγμένων", type="primary"):
                ...
                st.success("✅ Οι εγγραφές διαγράφηκαν επιτυχώς!")
                st.cache_data.clear()
                time.sleep(1)
                st.rerun()
        ...
        # --- Β. Η ΔΙΚΗ ΣΟΥ ΛΟΓΙΚΗ (ΑΝΑΖΗΤΗΣΗ & ΕΠΕΞΕΡΓΑΣΙΑ) ---
        st.subheader("📂 Αναζήτηση & Επεξεργασία Παραγωγής")
        ...
        if sel_hist_date:
            ...
            if selected_batch != "-- Επιλέξτε Παραγωγή --":
                ...
                # Dropdown για τον πελάτη από το πελατολόγιο
                ...
                with st.form(f"edit_batch_form_{batch_id}"):
                    ...
                    for col, label in zip(h_edit, ["Υλικό", "ml", "Lot 1", "Λήξη 1", "Lot 2", "Λήξη 2"]):
                        col.caption(label)
            ...
            # --- ΕΝΟΤΗΤΑ ΕΚΤΥΠΩΣΗΣ PDF (ΠΡΟΣΟΧΗ ΣΤΗΝ ΕΥΘΥΓΡΑΜΜΙΣΗ) ---
            if st.button("🖨️ Εκτύπωση PDF"):
                # Εδώ η μεταβλητή html_pro πρέπει να είναι σωστά ευθυγραμμισμένη κάτω από το 'if'
                html_pro = f"""
                <html>
                <body style="font-family: Arial;">
                    <h2>Δελτίο Παραγωγής: {selected_batch}</h2>
                    <table border='1' style='width:100%; border-collapse: collapse;'>
                        <tr><th>Υλικό</th><th>ML</th><th>Lot Number</th><th>Λήξη</th></tr>

...
def generate_scenario_forecast_pdf(data, now_str):
    ...
    # --- 📖 ΕΠΕΞΗΓΗΜΑΤΙΚΟΙ ΠΙΝΑΚΕΣ (χωρίς τύπους — μόνο σε τι εξυπηρετεί κάθε βήμα) ---
    pdf.add_page()
    ...
    section("Α. ΠΩΣ ΒΓΑΙΝΟΥΝ ΤΑ ΣΕΝΑΡΙΑ")
    explain_row("1. Εντοπισμός πραγματικών μηνών", "Βρίσκει ποιοι μήνες μέσα στο επιλεγμένο εύρος έχουν ήδη πραγματικά δεδομένα παραγωγής.")
    explain_row("2. Βαθμονόμηση ρυθμού", "Υπολογίζει, από τα πραγματικά νούμερα, πόσο δυνατός είναι κάθε γνωστός μήνας σε σχέση με τους άλλους.")
    explain_row("3. Προβολή υπόλοιπων μηνών", "Εκτιμά τις πωλήσεις στους μήνες που δεν έχεις ακόμα δεδομένα, βάσει του ρυθμού και του επιλεγμένου σεναρίου (Χειρότερο/Αναμενόμενο/Καλύτερο).")
    explain_row("4. Σταθερά έξοδα περιόδου", "Αθροίζει τα πραγματικά καταχωρημένα σταθερά έξοδα όλης της περιόδου· όπου λείπει καταχώρηση, χρησιμοποιείται ο πρώτος μήνας του εύρους ως προεπιλογή.")
    explain_row("5. Τελικό αποτέλεσμα", "Τζίρος (πραγματικός + προβλεπόμενος) μείον μεταβλητό κόστος μείον σταθερά έξοδα.")
    pdf.ln(3)
    ...
    explain_row("1. Υπολογισμός προτεραιότητας ανά κοκτέιλ", "Δίνεται μεγαλύτερη προτεραιότητα σε κοκτέιλ που είναι δημοφιλή αλλά έχουν χαμηλό κέρδος/τεμάχιο (Plowhorses) — εκεί μια μικρή αύξηση αποφέρει το μεγαλύτερο πρόσθετο κέρδος, με τον μικρότερο κίνδυνο αντίδρασης πελατών.")
    explain_row("2. Κατανομή της απαιτούμενης αύξησης", "Το ποσό που λείπει για να φτάσεις στο 0 μοιράζεται σε όλα τα κοκτέιλ ανάλογα με την προτεραιότητα του καθενός — όχι ισόποσα.")
...
with st.sidebar:
    ...
    MENU_GROUPS = {
        "🏭 Παραγωγή & Απόθεμα": [
            "📦 Lot Παραγωγής", "🏷️ Προσφορές & Εκπτώσεις", "📦 Αποθήκη", "🛒 Λίστα Αγορών", "🚚 Παραλαβές",
            "🧼 Συντήρηση & HACCP", "🧪 Δοκιμαστικές Παραγωγές",
        ],
        "📖 Συνταγές": [
            "📝 Νέα Συνταγή", "📊 Διαχείριση", "🔄 Αντικατάσταση",
        ],
        ...
        "📊 Επιχείρηση & Πωλήσεις": [
            ...
            "📦 Παραγγελίες B2B", "🧪 Προσομοίωση Πωλήσεων",
        ],
    }
...
# --- 📐 MARKUP & MARGIN: CABCLUB → ΑΝΤΙΠΡΟΣΩΠΟΣ → ΠΕΛΑΤΗΣ ---
    ...
    if df_rec.empty:
        ...
        # εμφάνιση, με pd.isna() που πιάνει ΚΑΘΕ τύπο κενής τιμής, ό,τι κι αν είναι η αιτία.
        def _clean_none(v):
            try:
                if v is None or pd.isna(v):
                    return "Μη εφικτό"
            except (TypeError, ValueError):
                pass  # π.χ. αν το pd.isna() σκάσει σε ασυνήθιστο τύπο, το αφήνουμε όπως είναι
            return v

        for _col in df_all_scenario.columns:
            df_all_scenario[_col] = df_all_scenario[_col].apply(_clean_none)
...
# --- 🎯 ΝΕΚΡΟ ΣΗΜΕΙΟ (BREAK-EVEN) ---
    ...
    if be_mode == "blended":
        ...
        # =====================================================================
        # 📉 ΣΕΝΑΡΙΟ ΜΕΙΩΣΗΣ ΤΙΜΩΝ ΣΕ ΕΠΙΛΕΓΜΕΝΑ ΚΟΚΤΕΪΛ (με πρόβλεψη αύξησης όγκου)
        # =====================================================================
        st.divider()
        st.subheader("📉 Σενάριο Μείωσης Τιμών σε Επιλεγμένα Κοκτέιλ")
        st.caption(
            "Δοκιμάζει την υπόθεση: αν μειώσουμε τιμή σε συγκεκριμένα κοκτέιλ, οι αντιπρόσωποι "
            "θα παραγγείλουν περισσότερο (καλύτερο δικό τους περιθώριο) ή θα προσελκύσουμε πελάτες "
            "που σήμερα θεωρούν την τιμή εμπόδιο. Επίλεξε ΕΣΥ ποια κοκτέιλ αφορά η μείωση — δεν "
        ...
        if not selected_discount_cocktails:
            ...
            if not _discount_items_base:
                ...
                st.dataframe(df_discount.style.apply(_hl_discount, axis=1), use_container_width=True, hide_index=True)
                ...
                for col, (label, res) in zip([dcol1, dcol2, dcol3], fc_results.items()):
                    with col:
                        ...
                        # 🆕 Πόσο % του συνολικού κενού καλύπτει η μείωση τιμών σε αυτά τα κοκτέιλ —
                        # ώστε να φαίνεται κατευθείαν αν μια "εξωφρενική" απαίτηση αύξησης όγκου
                        # οφείλεται στο ότι τα επιλεγμένα κοκτέιλ είναι απλά μικρό κομμάτι του συνόλου.
                        if res["profit"] < 0:
...
elif page == "🏷️ Προσφορές & Εκπτώσεις":
    st.header("🏷️ Προσφορές & Εκπτώσεις")
    st.caption("Εφαρμογή γενικής έκπτωσης πελάτη, δωρεάν τεμαχίων, ή ειδικών εκπτώσεων σε συγκεκριμένη παραγγελία — μεταφέρθηκε εδώ από το Πελατολόγιο, μιας και οι εκπτώσεις σε παραγγελίες εφαρμόζονται από την παραγωγή.")
...
# --- 10. ΠΕΛΑΤΟΛΟΓΙΟ (CRM - ΜΕ ΑΦΜ, ΕΚΠΤΩΣΗ & ΙΣΤΟΡΙΚΟ ΠΡΟΣΦΟΡΩΝ) ---
    ...
    # 3ο tab (Προσφορές & Εκπτώσεις) μεταφέρθηκε στην κατηγορία «Παραγωγή & Απόθεμα» — δική του καρτέλα πλέον
    tab_crm1, tab_crm2 = st.tabs([

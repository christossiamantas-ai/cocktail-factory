...
if res_log.data:
    ...
    # --- Α. ΜΑΖΙΚΗ ΔΙΑΓΡΑΦΗ (INTERACTIVE TABLE) ---
    # Προσθήκη στήλης για σωστή ταξινόμηση ημερομηνίας
    ...
    st.markdown("### 🗑️ Μαζική Επιλογή για Διαγραφή")
    st.caption("Τσεκάρετε τις γραμμές που θέλετε να σβήσετε μαζικά αριστερά στον πίνακα.")

    # Δημιουργία Interactive Πίνακα
    event = st.dataframe(
    ...
    if selected_indices:
        ...
        if st.button(f"🗑️ Διαγραφή {len(selected_indices)} επιλεγμένων εγγραφών", type="primary", use_container_width=True):
            try:
                for di in ids_to_delete:
                    supabase.table("production_log").delete().eq("id", di).execute()
                st.success("✅ Η διαγραφή ολοκληρώθηκε!")
                st.cache_data.clear()
                time.sleep(1)
                st.rerun()
            except Exception as e:
    ...
    if sel_hist_date:
        ...
        if selected_batch != "-- Επιλέξτε Παραγωγή --":
            ...
            # Dropdown Πελάτη (αντικατάσταση του text_input)
            try:
                res_c = supabase.table("customers").select("name").execute()
                customer_list = sorted([c["name"] for c in res_c.data]) if res_c.data else []
                if base_data["Πελάτης"] not in customer_list: customer_list.append(base_data["Πελάτης"])
            except:
                customer_list = [base_data["Πελάτης"]]
            
            new_cust = c1.selectbox("Πελάτης", options=customer_list, index=customer_list.index(base_data["Πελάτης"]), key=f"ed_cust_{batch_id}")

...
# --- 10. ΠΕΛΑΤΟΛΟΓΙΟ (CRM - ΜΕ ΑΦΜ, ΕΚΠΤΩΣΗ & ΙΣΤΟΡΙΚΟ ΠΡΟΣΦΟΡΩΝ) ---
        ...
        if not df_cust.empty:
            ...
            with col_crm_b:
                ...
                if res_prod.data:
                    ...
                    # 🚀 ΔΙΟΡΘΩΣΗ: Κρατάμε 1 γραμμή ανά μοναδικό "χτύπημα" κοκτέιλ (αγνοούμε τα πολλαπλά υλικά)
                        ...
                        if st.button("📄 Δημιουργία Απόλυτης Αναφοράς"):
                            with st.spinner("Υπολογισμός Κόστους & Κέρδους..."):
                                ...
                                if not df_sales_raw.empty:
                                    ...
                                    # 🚀 ΦΙΛΤΡΟ ΜΟΝΑΔΙΚΟΤΗΤΑΣ: 1 γραμμή ανά κοκτέιλ (αφαιρούμε τα υλικά)
                                ...
                                for o in orders_to_export:
                                    ...
                                    o_time_str = dt_parsed.strftime("%H:%M")
                                    
                                    html_content += f"""<tr class="order-date-row"><td colspan="7">📅 Παραγγελία: {date_formatted}</td></tr>"""
                                    
                                    if not df_sales.empty:
                                        # 🚀 ΦΙΛΤΡΑΡΟΥΜΕ ΚΑΙ ΜΕ ΤΗΝ ΩΡΑ ΓΙΑ ΝΑ ΜΗΝ ΕΝΩΝΟΝΤΑΙ ΟΙ ΠΑΡΑΓΓΕΛΙΕΣ ΤΗΣ ΙΔΙΑΣ ΜΕΡΑΣ ΣΤΟ PDF
                                        day_sales = df_sales[(df_sales['match_date'] == o_date_raw) & (df_sales['prod_time'] == o_time_str)]
                                        
                                        if day_sales.empty: # fallback αν υπάρχει μικρή απόκλιση
                                            day_sales = df_sales[df_sales['match_date'] == o_date_raw]
                                        
                                        if not day_sales.empty:
                                            grouped_day_sales = day_sales.groupby(["cocktail_name", "lot_cocktail", "prod_time"]).agg({
                                                "t_pcs": "sum", "f_pcs": "sum", "s_pcs": "sum", "s_pct": "max",
                                                "Theoretical_Revenue": "sum", "Total_Cost": "sum", "Profit": "sum",
                                                "sum_ml": "sum", "price_after_global": "first", "ingredient_name": "first"
                                            }).reset_index()
                            ...
                            st.info("💡 **Oδηγία:** Ανοίξτε το αρχείο στον browser σας και πατήστε **Ctrl+P** (Print) για να το αποθηκεύσετε ως PDF!")
                    
                    st.divider()

                    # --- ΛΙΣΤΑ ΠΑΡΑΓΓΕΛΙΩΝ (EXPANDERS) ΜΕ ΦΠΑ & ΑΚΡΙΒΗ ΩΡΑ ---
                    # 🚀 ΔΙΟΡΘΩΣΗ: Προσθέτουμε την ακριβή Ώρα στον τίτλο του expander για να μην φαίνονται ποτέ ως μία!
    ...
    # =========================================================================
    # 🚀 TAB 3: ΤΟ ΣΤΡΑΤΗΓΕΙΟ ΤΩΝ ΕΚΠΤΩΣΕΩΝ & ΠΡΟΣΦΟΡΩΝ (ΠΛΗΡΩΣ ΔΙΟΡΘΩΜΕΝΟ)
    # =========================================================================
    with tab_crm3:
            ...
            if res_orders.data:
                ...
                if sel_order_id:
                    ...
                    if res_prod.data:
                        ...
                        # 🚀 ΧΕΙΡΟΥΡΓΙΚΟ ΦΙΛΤΡΟ ΜΟΝΑΔΙΚΟΤΗΤΑΣ: Απορρίπτουμε τα διπλά υλικά και κρατάμε ΜΟΝΟ 1 γραμμή ανά κοκτέιλ/ώρα
                        df_prod_unique = df_prod.drop_duplicates(subset=["cocktail_name", "lot_cocktail", "prod_time"])
                        
                        # Απομονώνουμε μόνο τα κοκτέιλ που ανήκουν στην ώρα της συγκεκριμένης παραγγελίας
                        df_filtered_order = df_prod_unique[df_prod_unique['prod_time'] == order_time_str]
                        
                        # Fallback αν υπάρχει 1 λεπτό απόκλιση
                        if df_filtered_order.empty:
                            df_filtered_order = df_prod_unique
                        ...
                        with st.form("apply_gifts_and_discounts_form"):
                            ...
                            for idx, prow in df_filtered_order.iterrows():
                                ...
                                safe_key_suffix = f"{idx}_{c_name}_{lot_c}".replace("/", "_").replace("-", "_").replace(" ", "")
                                ...
                                # Αποθηκεύουμε τα φίλτρα για να ενημερώσουμε όλα τα υλικά μαζί
                            ...
                            if st.form_submit_button("💾 Εφαρμογή & Επανυπολογισμός Παραγγελίας", type="primary"):
                                ...
                                with st.spinner("Επανυπολογισμός αξιών..."):
                                    for key, vals in inputs.items():
                                        ...
                                        # 🚀 ΑΠΟΛΥΤΗ ΔΙΟΡΘΩΣΗ: Το update βρίσκει και ενημερώνει ΚΑΙ ΤΑ 4 ΥΛΙΚΑ της συνταγής ταυτόχρονα στη βάση!
                                        supabase.table("production_log").update({
                                            "free_pieces": f_pcs,
        ...
        if sel_cust_offers != "-- Επιλέξτε --":
            ...
            if res_orders.data:
                ...
                if sel_order_id:
                    ...
                    # 🚀 ΦΕΡΝΟΥΜΕ ΤΟ ΜΟΝΑΔΙΚΟ ID: Καμία ομαδοποίηση πλέον!
                    res_prod = supabase.table("production_log").select("id, cocktail_name, pieces, free_pieces, discounted_pieces, discount_pct, lot_cocktail, prod_time").eq("customer", sel_cust_offers).eq("prod_date", prod_date_str).execute()
                    
                    if res_prod.data:
                        df_prod = pd.DataFrame(res_prod.data).fillna(0)
                        df_prod['pieces'] = pd.to_numeric(df_prod['pieces'], errors='coerce').fillna(0).astype(int)
                        
                        # Κρατάμε μόνο τα κοκτέιλ που αναφέρονται στο κείμενο της παραγγελίας για να μην μπερδευτούμε με άλλες ώρες
                        valid_cocktails = [line.replace('•', '').split(' τμχ ')[1].split(' (Εκ των')[0].split(' (LOT:')[0].strip() for line in clean_display_details.split('\n') if 'τμχ' in line]
                        if valid_cocktails:
                            df_prod = df_prod[df_prod['cocktail_name'].isin(valid_cocktails)]

                        # Αντί να τα "ζουλάμε", τα εμφανίζουμε ακριβώς όπως κόπηκαν, με βάση το μοναδικό τους ID
                        with st.form("apply_gifts_and_discounts_form"):
                            inputs = {}
                            st.markdown("##### ⚙️ Ρυθμίσεις ανά Γραμμή Παραγωγής")
                            ...
                            for _, prow in df_prod.iterrows():
                                row_id = prow['id']  # <-- ΤΟ ΚΛΕΙΔΙ ΓΙΑ ΝΑ ΜΗΝ ΜΠΕΡΔΕΥΟΝΤΑΙ!
                                c_name = prow['cocktail_name']
                                lot_c = prow['lot_cocktail']
                                p_time = prow['prod_time']
                                t_pcs = int(prow['pieces'])
                                ...
                                f_pcs = c2.number_input("Δώρο", min_value=0, max_value=t_pcs, value=curr_free, step=1, key=f"f_{row_id}", label_visibility="collapsed")
                                s_pcs = c3.number_input("Τμχ Έκπτωσης", min_value=0, max_value=t_pcs, value=curr_s_pcs, step=1, key=f"s_{row_id}", label_visibility="collapsed")
                                s_pct = c4.number_input("% Έκπτωσης", min_value=0.0, max_value=100.0, value=curr_s_pct, step=1.0, key=f"p_{row_id}", label_visibility="collapsed")
                                
                                inputs[row_id] = {"c_name": c_name, "lot_c": lot_c, "t_pcs": t_pcs, "f_pcs": f_pcs, "s_pcs": s_pcs, "s_pct": s_pct}
                            ...
                            if st.form_submit_button("💾 Εφαρμογή & Επανυπολογισμός Παραγγελίας", type="primary"):
                                ...
                                with st.spinner("Επανυπολογισμός αξιών..."):
                                    for row_id, vals in inputs.items():
                                        t_pcs = vals["t_pcs"]
                                        f_pcs = vals["f_pcs"]
                                        s_pcs = vals["s_pcs"]
                                        s_pct = vals["s_pct"]
                                        ...
                                        # 🚀 ΑΠΟΛΥΤΗ ΑΚΡΙΒΕΙΑ: Αλλάζει ΜΟΝΟ τη συγκεκριμένη γραμμή βάσει ID
                                        supabase.table("production_log").update({
                                            "free_pieces": f_pcs,
                                            "discounted_pieces": s_pcs,
                                            "discount_pct": s_pct

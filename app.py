...
# --- 8. LOT ΠΑΡΑΓΩΓΗΣ (ΜΕ DROP-DOWN ΠΕΛΑΤΟΛΟΓΙΟ) ---
    ...
    # --- 9. ΙΣΤΟΡΙΚΟ ΠΑΡΑΓΩΓΗΣ & ΔΙΑΧΕΙΡΙΣΗ (ΜΠΛΟΚ 3 - ΔΙΟΡΘΩΜΕΝΟ) ---
elif page == "📜 Ιστορικό Παραγωγής":
    st.header("📜 Ιστορικό Παραγωγής & Διαχείριση")
    ...
    if res_log.data:
        ...
        st.subheader("🗑️ Μαζική Επιλογή για Διαγραφή")
        st.caption("Τσεκάρετε τις γραμμές αριστερά στον πίνακα και πατήστε το κουμπί διαγραφής.")
        ...
        # Κουμπί Διαγραφής (εμφανίζεται μόνο αν επιλεγούν γραμμές)
        selected_rows = event.selection.rows
        if selected_rows:
            ids_to_delete = df_display.iloc[selected_rows]['id'].tolist()
            if st.button(f"🗑️ Οριστική Διαγραφή {len(selected_rows)} εγγραφών", type="primary"):
        ...
        # --- Β. ΕΠΕΞΕΡΓΑΣΙΑ ΑΝΑ ΠΑΡΤΙΔΑ (Η ΔΙΚΗ ΣΟΥ ΛΟΓΙΚΗ) ---
        st.subheader("📂 Επεξεργασία Συγκεκριμένης Παραγωγής")
        ...
        if sel_hist_date:
            ...
            selected_batch = st.selectbox("🛠️ Διόρθωση Παραγωγής / Προσθήκη LOT 2:", options)
            ...
            if selected_batch != "-- Επιλέξτε Παραγωγή --":
                ...
                # --- ΦΟΡΤΩΣΗ ΠΕΛΑΤΩΝ ΓΙΑ ΤΟ DROP-DOWN ---
                try:
                    res_cust = supabase.table("customers").select("name").execute()
                    c_list = sorted([c["name"] for c in res_cust.data]) if res_cust.data else []
                    if base_data["Πελάτης"] not in c_list: c_list.append(base_data["Πελάτης"])
                except:
                    c_list = [base_data["Πελάτης"]]
                ...
                # Υπολογισμός συστατικών βάσει αλλαγών
                display_ingredients = []
                ...
                with st.form(f"edit_batch_form_{batch_id}"):
                    ...
                    for col, lbl in zip(h_edit, ["Υλικό", "ml", "Lot 1", "Λήξη 1", "Lot 2", "Λήξη 2"]): col.caption(lbl)

                    final_updated = []
                    for i, itm in enumerate(display_ingredients):
                        raw_lot = str(itm["Lot"])
                        raw_exp = str(itm["Exp"])
                        lot_p = raw_lot.split(" / ") if " / " in raw_lot else [raw_lot, ""]
                        exp_p = raw_exp.split(" / ") if " / " in raw_exp else [raw_exp, ""]
                        while len(lot_p) < 2: lot_p.append("")
                        while len(exp_p) < 2: exp_p.append("")

                        r = st.columns([2, 1, 1.2, 1.2, 1.2, 1.2])
                        r[0].write(f"**{itm['Υλικό']}**")
                        r[1].write(f"{itm['ML']:.0f}")
                        lt1 = r[2].text_input("L1", value=lot_p[0], key=f"ed_l1_{batch_id}_{i}", label_visibility="collapsed")
                        ex1 = r[3].text_input("E1", value=exp_p[0], key=f"ed_e1_{batch_id}_{i}", label_visibility="collapsed")
                        lt2 = r[4].text_input("L2", value=lot_p[1], key=f"ed_l2_{batch_id}_{i}", label_visibility="collapsed")
                        ex2 = r[5].text_input("E2", value=exp_p[1], key=f"ed_e2_{batch_id}_{i}", label_visibility="collapsed")
                        
                        f_lot = lt1 if not lt2 else f"{lt1} / {lt2}"
                        f_exp = ex1 if not ex2 else f"{ex1} / {ex2}"
                        final_updated.append({"ing": itm["Υλικό"], "ml": itm["ML"], "lot": f_lot, "exp": f_exp})
                    ...
                    if b_save.form_submit_button("💾 Αποθήκευση Αλλαγών", type="primary"):
                        ...
                        for fd in final_updated:
                            ...
                            new_batch.append({
                                ...
                                "customer": new_cust, "cocktail_name": new_cock, "lot_cocktail": new_lot_c, "pieces": int(new_pcs), 

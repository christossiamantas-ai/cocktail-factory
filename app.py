...
if not res_chart.data:
    ...
    # 2. ΚΑΘΑΡΙΣΜΟΣ ΔΕΔΟΜΕΝΩΝ: Επειδή η παραγωγή καταγράφεται ανά υλικό, 
    # κρατάμε μόνο μία εγγραφή ανά παρτίδα (ίδια μέρα, ώρα, πελάτης, κοκτέιλ)
    # για να μην μετρήσουμε τα τεμάχια πολλές φορές!
    df_unique_sales = df_chart.groupby(["prod_date", "prod_time", "customer", "cocktail_name"]).agg(
        pieces=("pieces", "first"),
        sold_price=("sold_price", "first"), # Τιμή πώλησης (μετά την έκπτωση)
        unit_cost=("unit_cost", "first")    # Κόστος παραγωγής ανά τεμάχιο
    ).reset_index()
    
    # Αν κάποια πεδία είναι κενά (π.χ. παλιές εγγραφές), τα κάνουμε 0
    ...
    fig = px.bar(
        ...
        color_continuous_scale="Viridis", # Όμορφη χρωματική παλέτα
        labels={"cocktail_name": "Ονομασία Κοκτέιλ", selected_metric: selected_metric},
        title=f"Κατάταξη Κοκτέιλ βάσει: {selected_metric}"
    )
    
    # Μορφοποίηση του γραφήματος για να φαίνεται επαγγελματικό
    fig.update_traces(texttemplate='%{text:.1f}', textposition='outside')
    fig.update_layout(
        xaxis_tickangle=-45,
        height=500,
        margin=dict(t=50, b=100),
        plot_bgcolor="rgba(0,0,0,0)" # Διάφανο background
    )
    
    # Εμφάνιση στο Streamlit
    st.plotly_chart(fig, use_container_width=True)
    
    # (Προαιρετικό) Εμφάνιση και σε μορφή Πίνακα για γρήγορη ανάγνωση

...
with st.sidebar:
    ...
    page = st.radio(
        ...
        [
            ...
            "📦 Lot Παραγωγής", "📈 Dashboard", "👥 Πελατολόγιο", "🧼 Συντήρηση & HACCP","🧪 Προσομοίωση Πωλήσεων", "🛒 Λίστα Αγορών", "🚚 Παραλαβές", "🧪 Δοκιμαστικές Παραγωγές", "📊 Οικονομικά"
        ],
        key="main_page"
    )
...
# =====================================================================
# ΝΕΟΣ ΚΩΔΙΚΑΣ: ΚΟΛΛΑΣ ΑΠΟ ΕΔΩ ΚΑΙ ΚΑΤΩ (ΤΟ elif ΑΚΟΥΜΠΑΕΙ ΤΕΡΜΑ ΑΡΙΣΤΕΡΑ)
# =====================================================================
    ...
    if not df_fin.empty:
        ...
        if total_margin < 20.0:
            m4.metric("🚨 Καθαρό Κέρδος", f"{total_profit:,.2f} €", f"{total_margin:.1f}% Margin", delta_color="inverse")
        else:
        ...
        df_accounting_view = df_batches.rename(columns={
            "prod_date": "📅 Ημερομηνία", "customer": "👤 Πελάτης", "cocktail_name": "🍹 Cocktail",
            "lot_cocktail": "🔢 LOT", "pieces": "📦 Τμχ"
        })[["📅 Ημερομηνία", "👤 Πελάτης", "🍹 Cocktail", "🔢 LOT", "📦 Τμχ", "Κόστος Παραγωγής (€)", "Έσοδα (€)", "Καθαρό Κέρδος (€)", "Περιθώριο Κέρδους (%)"]]

        st.dataframe(
            df_accounting_view.style.format({
                "Κόστος Παραγωγής (€)": "{:,.2f} €", "Έσοδα (€)": "{:,.2f} €",
                "Καθαρό Κέρδος (€)": "{:,.2f} €", "Περιθώριο Κέρδους (%)": "{:.1f} %"
            }),
            use_container_width=True, hide_index=True
        )
        ...
        st.download_button(
            label="📥 Λήψη Οικονομικής Αναφοράς (Excel / CSV)",
            data=csv_data,

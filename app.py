...
# --- 7. DASHBOARD (ΤΕΛΙΚΗ & ΚΑΘΑΡΗ ΕΚΔΟΣΗ - ΧΩΡΙΣ ΣΦΑΛΜΑΤΑ ΑΡΙΘΜΗΣΗΣ) ---
elif page == "📈 Dashboard":
    ...
    if res_log.data and res_rec.data:
        ...
        if sel_customer != "ΟΛΟΙ ΟΙ ΠΕΛΑΤΕΣ":
            ...
            if not df_orders.empty: df_orders = df_orders[df_orders['customer_name'] == sel_customer]
        if sel_month != "ΟΛΟΙ ΟΙ ΜΗΝΕΣ":
            df_filtered = df_filtered[df_filtered['Month_Year'] == sel_month]
            if not df_orders.empty: df_orders = df_orders[df_orders['Month_Year'] == sel_month]
        ...
        if not df_filtered.empty:
            ...
            df_perf["% Τζίρου"] = (df_perf["Τζίρος"] / tot_rev_perf * 100) if tot_rev_perf > 0 else 0
            df_perf["% Κέρδους"] = (df_perf["Κέρδος"] / tot_prof_perf * 100) if tot_prof_perf > 0 else 0

            # Χρωματισμός χωρίς hide_index για να μην σκάει
            st.dataframe(
                df_perf.sort_values("Κέρδος", ascending=False).rename(columns={"cocktail_name": "Cocktail"}).style.format({
                    "Τεμάχια": "{:.0f}",
                    "Τζίρος": "{:.2f} €",
                    "Κέρδος": "{:.2f} €",
                    "% Τζίρου": "{:.1f}%",
        ...
        # --- ΑΝΑΛΥΤΙΚΟΣ ΠΙΝΑΚΑΣ & ΑΝΑΛΥΣΗ ΠΕΛΑΤΗ ---
        with st.expander("📄 Αναλυτικά & Πληροφορίες"):

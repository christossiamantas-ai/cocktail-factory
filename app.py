...
with st.sidebar.expander("📖 Οδηγίες Χρήσης (SOS)"):
    ...
    * **Διαγραφή Ενός Κοκτέιλ:** Πηγαίνετε στην καρτέλα **"Παραγωγή & LOT"**. Επιλέξτε την εγγραφή και πατήστε το κουμπί διαγραφής εκεί.
    ...
    ### ⚠️ Κανόνες Ασφαλείας
    * **Αλλαγή Ημερομηνίας:** Αν αλλάξετε ημερομηνία σε μια εγγραφή παραγωγής, **μην διαγράψετε** την παραγγελία από το Πελατολόγιο αργότερα (δεν θα βρει τα υλικά για να τα σβήσει). 
    ...
    * **Μορφή Ημερομηνιών:** Σε όλα τα LOT και τις ημερομηνίες, χρησιμοποιείτε πάντα τη μορφή **ΗΗ/ΜΜ/ΕΕΕΕ** (π.χ. 20/05/2026).
    ...
    ### 🛡️ HACCP
    * Μην ξεχνάτε τον **Καθημερινό Έλεγχο**. Σε περίπτωση προβλήματος (π.χ. θερμοκρασία εκτός ορίων), καταγράφετε υποχρεωτικά την "Διορθωτική Ενέργεια" στο πεδίο Παρατηρήσεις.
    """)
    st.markdown("### 📉 Κίνδυνοι Αλλαγών (Ιστορικότητα)")
    st.warning("""
    **ΠΡΟΣΟΧΗ στις τροποποιήσεις παλιών δεδομένων:**
    ...
    * **Αλλαγή Πρώτης Ύλης:** Αν μετονομάσετε μια πρώτη ύλη, το σύστημα θα τη θεωρήσει "νέα". Τα παλιά LOT θα διατηρήσουν το παλιό όνομα (ορθώς, για λόγους ιχνηλασιμότητας).
    
    **💡 Συμβουλή:** Αν θέλετε να κάνετε αλλαγή τιμής ή δοσολογίας για μια "νέα εποχή" (π.χ. ανατίμηση), προτιμήστε να δημιουργήσετε **νέα συνταγή** (π.χ. "Zombie v2") αντί να αλλάξετε την υπάρχουσα. Έτσι το ιστορικό σας παραμένει πεντακάθαρο!
    """)
...
# --- 10. ΠΕΛΑΤΟΛΟΓΙΟ (CRM - ΜΕ ΑΦΜ, ΕΚΠΤΩΣΗ & ΙΣΤΟΡΙΚΟ ΠΡΟΣΦΟΡΩΝ) ---
    ...
    with tab_crm3:
        ...
        if sel_cust_offers != "-- Επιλέξτε --":
            ...
            # --- ΕΝΟΤΗΤΑ 2: ΕΜΠΟΡΙΚΗ ΠΑΡΕΜΒΑΣΗ ΣΕ ΠΑΡΑΓΓΕΛΙΑ (ΔΩΡΑ & ΕΚΠΤΩΣΕΙΣ) ---
            st.markdown("### 🎁 2. Εφαρμογή Δώρων & Ειδικών Εκπτώσεων σε Παραγγελία")
            st.write("Επιλέξτε μια παραγγελία. Για κάθε κοκτέιλ της, μπορείτε να ορίσετε δωρεάν τεμάχια ή/και ειδική έκπτωση σε συγκεκριμένο αριθμό τεμαχίων (π.χ. 15% στα πρώτα 24).")
            ...
            if res_orders.data:
                ...
                if sel_order_id:
                    ...
                    # Προβολή καθαρών λεπτομερειών (κρύβουμε τις παλιές σημειώσεις δώρων για να μην μπερδεύουν)
                            ...
                            if st.form_submit_button("💾 Εφαρμογή & Επανυπολογισμός Παραγγελίας", type="primary"):
                                ...
                                with st.spinner("Επανυπολογισμός αξιών..."):
                                    for key, vals in inputs.items():
                                        ...
                                        # Ασφάλεια: Τα δώρα + τα εκπτωτικά τεμάχια δεν μπορούν να ξεπερνούν το σύνολο
                                        if f_pcs + s_pcs > t_pcs:
                                            s_pcs = t_pcs - f_pcs # Περιορίζουμε τα εκπτωτικά για να μη βγει αρνητικό

...
def delete_order_and_production_safely(order_id, customer_name, created_at_timestamp, order_details):
    ...
    # 3. Φέρνουμε όλες τις γραμμές (select("*") για να είμαστε σίγουροι ότι παίρνουμε όλα τα πεδία)
    res_prod = supabase.table("production_log").select("*").eq("customer", customer_name).eq("prod_date", order_date).execute()
    
    if res_prod.data:
        for row in res_prod.data:
            # Χρησιμοποιούμε το row.get('cocktail_name') για να μην "σκάει" αν λείπει το πεδίο
            cocktail_name = row.get('cocktail_name', 'Άγνωστο')
            
            # Έξυπνος έλεγχος
            if str(cocktail_name) in str(order_details):
                # Διαγραφή με βάση το id (το id υπάρχει πάντα)
                row_id = row.get('id')
                if row_id:
                    supabase.table("production_log").delete().eq("id", row_id).execute()

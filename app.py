...
# --- 8. LOT ΠΑΡΑΓΩΓΗΣ (ΜΕ DROP-DOWN ΠΕΛΑΤΟΛΟΓΙΟ) ---
    ...
    # --- 4. ΙΣΤΟΡΙΚΟ & ΔΙΑΧΕΙΡΙΣΗ (ΜΟΝΟ ΔΙΑΧΕΙΡΙΣΗ - ΧΩΡΙΣ PDF) ---
    st.divider()
    st.subheader("📂 Ιστορικό Παραγωγής & Διαχείριση")
    
    # 1. ΦΟΡΤΩΣΗ ΔΕΔΟΜΕΝΩΝ ΑΠΟ ΤΗ ΒΑΣΗ
    ...
    if res_log.data:
        ...
        df_display = df_all_logs.sort_values(by=['Date_Sort', 'prod_time'], ascending=False)

        # --- Α. ΜΑΖΙΚΗ ΔΙΑΓΡΑΦΗ ΜΕ CHECKBOXES ---
        st.markdown("### 🗑️ Μαζική Επιλογή")
        st.caption("Επιλέξτε γραμμές αριστερά και πατήστε 'Διαγραφή Επιλεγμένων'.")
        ...
        event = st.dataframe(
            ...
            column_config={
                "id": None, 
                "prod_date": "Ημερομηνία", "prod_time": "Ώρα",
                "customer": "Πελάτης", "cocktail_name": "Cocktail",
                "pieces": "Τμχ", "lot_cocktail": "LOT"
            }
        )
        ...
        if selected_indices:
            ...
            if st.button(f"🗑️ Διαγραφή {len(selected_indices)} εγγραφών", type="primary"):
        ...
        # --- Β. ΕΠΕΞΕΡΓΑΣΙΑ ΑΝΑ ΠΑΡΤΙΔΑ (DROP-DOWN ΠΕΛΑΤΗ) ---
        ...
        if sel_hist_date:
            ...
            batches = df_past.groupby(['prod_time', 'Πελάτης', 'Cocktail', 'LOT_Cocktail']).groups
            options = ["-- Επιλέξτε Παραγωγή --"] + [f"🍹 {c} | 👤 {p} | 🕒 {t} | LOT: {l}" for (t, p, c, l) in batches.keys()]
            selected_batch = st.selectbox("🛠️ Επεξεργασία Συγκεκριμένης Παραγωγής:", options)
            
            if selected_batch != "-- Επιλέξτε Παραγωγή --":
                batch_mapping = {f"🍹 {c} | 👤 {p} | 🕒 {t} | LOT: {l}": list(idx) for (t, p, c, l), idx in batches.items()}
                ...
                new_cock = c2.selectbox("Cocktail", options=list(df_rec["Ονομα"].unique()), index=list(df_rec["Ονομα"].unique()).index(base_data["Cocktail"]) if base_data["Cocktail"] in list(df_rec["Ονομα"].unique()) else 0, key=f"ed_cock_{batch_id}")
                ...
                with st.form(f"edit_form_{batch_id}"):
                    ...
                    for i, idx in enumerate(row_indices):
                        ...
                        lot_parts = str(r_d["Lot Number"]).split(" / ") if " / " in str(r_d["Lot Number"]) else [str(r_d["Lot Number"]), ""]
                        while len(lot_parts) < 2: lot_parts.append("")
                        
                        lt1 = r[2].text_input("L1", value=lot_parts[0], key=f"l1_{batch_id}_{i}")
                        lt2 = r[4].text_input("L2", value=lot_parts[1], key=f"l2_{batch_id}_{i}")
                    ...
                    if st.form_submit_button("💾 Αποθήκευση Αλλαγών", type="primary"):
                        ...
                        for fd in final_updated:
                            new_batch.append({
                                ...
                                "ingredient_name": fd["ing"], "total_ml": fd["ml"], "lot_number": fd["lot"]
                            })

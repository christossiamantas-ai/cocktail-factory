...
# --- 8. LOT ΠΑΡΑΓΩΓΗΣ (ΜΕ DROP-DOWN ΠΕΛΑΤΟΛΟΓΙΟ & SMART CART) ---
    ...
    # ==========================================
        ...
        # ==========================================
        with tab_forward:
            st.markdown("#### 🔬 Έλεγχος & Διασταύρωση Παρτίδων")
            
            # Αρχικοποίηση session state για το multi-recall
            ...
            # Επιλογή Κοκτέιλ για διασταύρωση
            cocktails_in_log = sorted([c for c in df_trace['cocktail_name'].unique() if str(c).strip() not in ['nan', 'None', '']])
            
            c1, c2 = st.columns([2, 1])
            sel_cocktail = c1.selectbox("Επιλογή ελαττωματικού κοκτέιλ:", ["--"] + cocktails_in_log, key="sel_c")
            sel_date = c2.text_input("Ημερομηνία:", placeholder="DD/MM/YYYY")
            ...
            if st.session_state.recall_stack:
                ...
                if st.button("🧹 Καθαρισμός ανάλυσης"):
                    st.session_state.recall_stack = []
                    st.rerun()
